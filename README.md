...
#### sample model list
You can either use the clone feature above or opt a sample GGUF straight from the lazy list (download ready) by:
```
cgg s
```
More options might be found on list since it can be updated independently (by json approach without affecting the code).